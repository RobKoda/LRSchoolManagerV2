...
namespace LRSchoolV2.Reporting.Customers.CustomerQuotes;

public class CustomerQuoteReport(CustomerQuote inCustomerQuote, IEnumerable<CustomerQuoteItem> inCustomerQuoteItems)
{
    private const string TemplateFileName = "CustomerQuoteTemplate.docx";
    private string ExportFileNameNoExtension => $"{inCustomerQuote.Number} - {inCustomerQuote.QuoteCustomerName} - {inCustomerQuote.Date:yyyy-MM-dd}.";
    ...
    private void ReplaceTexts(MainDocumentPart? inExportMainPart)
    {
        ...
        {
            textElement.ReplaceIfFound("[QuoteNumber]", inCustomerQuote.Number);
            textElement.ReplaceIfFound("[QuoteDate]", inCustomerQuote.Date.ToString("dd/MM/yyyy"));
            textElement.ReplaceIfFound("[CustomerName]", inCustomerQuote.QuoteCustomerName);
            textElement.ReplaceIfFound("[CustomerAddress]", inCustomerQuote.QuoteCustomerAddress);
        }
    }
    ...
    private void FillTable(MainDocumentPart? inExportMainPart)
    {
        ...
        lastRowCells.WriteCell(lastRowCells.Count - 1, $"{inCustomerQuote.GetTotalToPayFromItems(inCustomerQuoteItems):0.00}€", JustificationValues.Right, true, 24);
    }
}