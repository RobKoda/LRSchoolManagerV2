...
namespace LRSchoolV2.Reporting.Consultants.ConsultantInvoices;

public class ConsultantInvoiceReport(ConsultantInvoice inConsultantInvoice, IEnumerable<ConsultantInvoiceItem> inConsultantInvoiceItems, byte[] inConsultantInvoiceDocument)
{
    private string ExportFileNameNoExtension => $"{inConsultantInvoice.Number} - {inConsultantInvoice.Consultant.GetFullName()} - {inConsultantInvoice.Date:yyyy-MM-dd}.";
    ...
    private async Task<WordprocessingDocument> CreateExportFileAsync()
    {
        ...
        var stream = new MemoryStream(inConsultantInvoiceDocument);
    ...
    private void ReplaceTexts(MainDocumentPart? inExportMainPart)
    {
        ...
        {
            textElement.ReplaceIfFound("[InvoiceNumber]", inConsultantInvoice.Number);
            textElement.ReplaceIfFound("[InvoiceDate]", inConsultantInvoice.Date.ToString("dd/MM/yyyy"));
        }
    }
    ...
    private void FillTable(MainDocumentPart? inExportMainPart)
    {
        ...
        for (var i = 0 ; i < inConsultantInvoiceItems.Count() ; i++)
        {
            var item = inConsultantInvoiceItems.OrderBy(inItem => inItem.Denomination).ElementAt(i);
        ...
        lastRowCells.WriteCell(lastRowCells.Count - 1, $"{inConsultantInvoice.GetTotalToPayFromItems(inConsultantInvoiceItems):0.00}€", JustificationValues.Right, true, 24);
    }
}